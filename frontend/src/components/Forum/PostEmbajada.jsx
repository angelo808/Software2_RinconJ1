...
const PostEmbajada = ({ post, setPosts, posts }) => {
  ...
  const handleAddComment = async () => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:5001/api/postsEmbajada/${post._id}/comment`,
  ...
  const handleReaction = async (reaction) => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:5001/api/postsEmbajada/${post._id}/reactions`, body);
  ...
  const reportPost = async () => {
    try {
      const response = await axios.put(`http://localhost:5001/api/postsEmbajada/${post._id}/report`)
      console.log(response.data)
  ...
  return (
    <Box>
      ...
      {comments.map((comment) => (
        <Comment key={comment._id} comment={comment} type={'EMBASSY'}/>
      ))}
